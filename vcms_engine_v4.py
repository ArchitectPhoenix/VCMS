...
# =============================================================================
# Passive Dynamics (no game interaction)
# =============================================================================

def run_vcms_v4_passive(params, n_steps, item_signal, dt=0.05):
    """
    Evolve VCMS state during passive possession (no game interaction).

    Models an agent passively possessing an item. The item signal integrates
    into v_level via alpha (V-channel), facilitation accumulates via
    facilitation_rate (M-channel), and budget replenishes (B-channel).
    No strain accumulation during possession — strain is computed at
    valuation time from the anticipated topology change of losing the item.

    Uses the same rate parameters as run_vcms_v4 but on a passive timescale:
    no round structure, no observables, no output decisions.

    v_level starts at 0 (pre-item baseline) and converges toward
    min(1.0, v_rep × item_signal) via alpha EMA. After n steps:
    v_level ≈ v_group × (1 - (1-alpha)^n).

    Args:
        params: Any object with alpha, v_rep, facilitation_rate,
                b_initial, b_replenish_rate attributes (VCMSParams or
                AgentParams both work).
        n_steps: Number of passive time steps.
        item_signal: Normalized item presence signal in [0, 1].
        dt: Time step size (default 0.05).

    Returns:
        dict with v_level, m_eval, B (evolved state after n_steps).
    """
    p = params
    v_group = min(1.0, p.v_rep * item_signal)

    v_level = 0.0       # pre-item baseline (item not yet integrated)
    m_eval = 0.0         # no facilitation initially
    B = p.b_initial      # starting budget

    for step in range(n_steps):
        # V: item signal integrates into v_level (EMA from 0 toward v_group)
        v_level = p.alpha * v_group + (1.0 - p.alpha) * v_level

        # M_eval: facilitation accumulates from positive experience of possession
        m_eval += dt * p.facilitation_rate * item_signal

        # B: replenishes from positive experience (no depletion in passive mode)
        B += dt * p.b_replenish_rate * item_signal
